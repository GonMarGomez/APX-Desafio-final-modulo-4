...
function getBienvenidaContent(){
  return fetch("https://cdn.contentful.com/spaces/6i04s7t927m9/environments/master/entries?access_token=e2mGaPLdxAvyOV8VXo5YtwyPVaindzvXp9_LmWw_9mw&content_type=bienvenida")
    .then((res) => {
      return res.json();
    })
    .then((data) => {
  ...
  function presentacionContent(params) {
    ...
    template.content.querySelector(".contenido__mi-nombre").textContent = params.titulo
      ...
      .then((data) => {
        const fieldsCollection = data.items.map((items) => {
          const obj = {
            ...
            parrafo: items.fields.parrafo.content.content,
